...
router.get("/my-deputies", useAuth, useEmsAuth, async (req: IRequest, res: Response) => {
  const deputies = await processQuery("SELECT * FROM `ems-fd` WHERE `user_id` = ?", [req.user?.id]);

  return res.json({ deputies, status: "success" });
});

router.post("/my-deputies", useAuth, useEmsAuth, async (req: IRequest, res: Response) => {
  const { name } = req.body;
  const id = uuidv4();
...
router.delete("/my-deputies/:id", useAuth, useEmsAuth, async (req: IRequest, res: Response) => {
  const { id } = req.params;
...
router.get("/status/:id", useAuth, useEmsAuth, async (req: IRequest, res: Response) => {
  const { id } = req.params;
...
router.put("/status/:id", useAuth, useEmsAuth, async (req: IRequest, res: Response) => {
  const { id } = req.params;
...
router.get("/medical-records/:name", useAuth, useEmsAuth, async (req: IRequest, res: Response) => {
...
async function useEmsAuth(req: IRequest, res: Response, next: NextFunction): Promise<void> {
