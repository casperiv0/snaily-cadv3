...
router.post("/my-officers", useAuth, usePermission(["leo"]), async (req: IRequest, res: Response) => {
...
router.put("/status/:id", useAuth, usePermission(["leo", "dispatch"]), async (req: IRequest, res: Response) => {
  const { id } = req.params;
...
router.get("/departments", useAuth, usePermission(["leo", "dispatch"]), async (_req: IRequest, res: Response) => {
...
/* searches */
router.post("/search/plate", useAuth, usePermission(["leo", "dispatch"]), async (req: IRequest, res: Response) => {
...
router.get("/search/names", useAuth, usePermission(["leo", "dispatch"]), async (_req, res: Response) => {
...
router.post("/search/name", useAuth, usePermission(["leo", "dispatch"]), async (req: IRequest, res: Response) => {
  const { name } = req.body;
  ...
  if (name) {
    ...
    const vehicles = await processQuery("SELECT * FROM `registered_cars` WHERE `citizen_id` = ?", [citizenId]);
    const weapons = await processQuery("SELECT * FROM `registered_weapons` WHERE `citizen_id` = ?", [citizenId]);
    const warnings = await processQuery("SELECT * FROM `written_warnings` WHERE `citizen_id` = ?", [citizenId]);
...
router.post("/search/weapon", useAuth, usePermission(["leo", "dispatch"]), async (req: IRequest, res: Response) => {
  const { serialNumber } = req.body;

  if (serialNumber) {
    const weapon = await processQuery("SELECT * FROM `registered_weapons` WHERE `serial_number` = ?", [serialNumber]);
...
router.post("/note/:citizenId", useAuth, usePermission(["leo", "dispatch"]), async (req: IRequest, res: Response) => {
